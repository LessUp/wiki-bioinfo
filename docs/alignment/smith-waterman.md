...
pagination_label: Smith-Waterman 算法
---
...
1. 如果 `H[i][j]` 来自 `H[i-1][j-1] + s(x_i, y_j)`：
   - 将 `x_i` 和 `y_j` 对齐
   - 移动到 `H[i-1][j-1]`
2. 如果 `H[i][j]` 来自 `H[i-1][j] + g`：
   - 将 `x_i` 与 gap 对齐
   - 移动到 `H[i-1][j]`
3. 如果 `H[i][j]` 来自 `H[i][j-1] + g`：
   - 将 gap 与 `y_j` 对齐
   - 移动到 `H[i][j-1]`
4. 如果 `H[i][j] = 0`：停止回溯
...
这个例子展示了 Smith-Waterman 如何找到局部高相似片段，而不需要考虑整个序列的对齐。
