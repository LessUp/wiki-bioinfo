...
<SummaryBox
  summary="概率算法是处理生物数据中噪声与不确定性的核心工具，它们不给出确定性的答案，而是量化不同解释的可能性。"
  bullets={[
    '理解概率模型如何表示生物学知识先验',
    '掌握从观测数据推断隐藏状态的统计方法',
  ]}
...
- **测序错误**：NGS reads 错误率 0.1-1%
- **生物变异**：SNP、indel、结构变异是自然存在的
- **采样噪声**：RNA-seq 表达量受采样深度影响
- **未知因素**：许多生物学过程尚未完全理解

确定性方法（如简单的规则匹配）难以处理这些问题。概率方法的优势：
...
即使检测准确率很高，阳性结果只有 50% 概率真正患病（因为疾病罕见）。
...
**定义**：PWM 表示 motif 在每个位置对不同碱基/氨基酸的偏好。
...
**问题**：当数据有缺失或隐变量时，直接 MLE 困难。

**策略**：
1. **E-step**：给定当前参数，计算隐变量的期望
2. **M-step**：基于期望，最大化似然更新参数
3. 迭代直到收敛
...
随机化算法在执行过程中做出随机决策。乍看之下这似乎是"灾难性的配方"，但实际上随机化算法在很多情况下非常有用。
...
详见 [Motif Discovery算法](../models/motif-discovery-algorithms.md)。
...
不一定。需要考虑多重检验、效应大小、先验知识。

### 贝叶斯方法总是更好

不是。贝叶斯方法需要先验，计算成本高。在某些场景下，频率方法更合适。

### MCMC 总是收敛

不是。MCMC 可能陷入局部最优，需要诊断收敛性。
...
- [PWM/PSSM 与 motif](../models/pwm-pssm.md)
