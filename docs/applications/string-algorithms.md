...
<SummaryBox
  summary="字符串算法与索引结构是生物信息学的基础设施，它们使得在人类基因组（3Gb）甚至更大参考序列上进行快速搜索成为可能。"
  bullets={[
    '理解索引结构的设计权衡：查询速度 vs 索引大小 vs 构建时间',
    '掌握从朴素搜索到高级索引的演进逻辑',
  ]}
/>
...
生物序列本质上是字符串（A、T、C、G 或 20 种氨基酸），因此：

- **规模巨大**：人类基因组 3Gb，参考数据库更大
- **查询频繁**：每次 NGS 分析需要映射数十亿 reads
- **实时性要求**：临床诊断需要快速结果
- **内存限制**：索引必须能装入内存

朴素字符串搜索（逐字符比较）时间复杂度 O(nm)，其中 n 是文本长度，m 是模式长度。对于人类基因组，这种方法完全不可行。

字符串算法与索引的目标是：

- **加速搜索**：从 O(n) 降到 O(log n) 或 O(1)
- **支持复杂查询**：近似匹配、通配符、间隙
- **节省内存**：压缩索引以处理大规模数据
- **支持更新**：增量式索引更新
...
**定义**：Trie 是一种树形结构，每个节点代表一个字符，从根到叶的路径代表一个字符串。
...
**问题**：空间复杂度 O(n²)，对于长序列不可行。

### 3. Suffix Tree（后缀树）

**定义**：Suffix Trie 的压缩版本，合并单分支路径。
...
**问题**：统计文本中所有 k-mer 的出现频率。

**方法**：
- **Hash 表**：简单但内存密集
- **Bloom filter**：概率性，可能误报
...
不是。索引应该在查询速度和内存使用之间平衡。FM-index 通过压缩实现了很好的权衡。

### Suffix Array 总是比 Suffix Tree 好

不一定。Suffix Tree 支持某些操作（如最长公共子串查询）更直接，但实现更复杂。
...
- [BWA 与 minimap2](../alignment/bwa-minimap2.md)
