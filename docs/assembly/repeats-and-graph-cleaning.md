...
| 算法 | 时间复杂度 | 空间复杂度 | 主要参数 |
|------|-----------|-----------|---------|
| Tip Removal | $O(|V| + |E|)$ | $O(L_{max})$ | $L_{max}$, $C_{min}$ |
| Bubble 识别 | $O(|V| \cdot d^2 \cdot L_{bubble})$ | $O(L_{bubble})$ | $L_{bubble}$, $S_{min}$ |
| Bubble 合并 | $O(|B| \cdot L_{bubble})$ | $O(L_{bubble})$ | coverage 比值 |
| 线性压缩 | $O(|V| + |E|)$ | $O(1)$ | 无 |
| 异常检测 | $O(|E|)$ | $O(1)$ | $Z_{thresh}$ |
