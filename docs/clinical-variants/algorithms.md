---
sidebar_position: 5
description: 临床变异解释中的核心算法：从优先级排序到 ACMG 分类、致病性预测与 CNV 检测。
---
...
- **候选变异过多**：一个外显子测序可能产生数万变异，需要高效筛选；
- **证据多维且异质**：频率、功能预测、表型匹配、家系分离等需要整合；
- **不确定性量化**：需要评估证据强度而非简单二分类；
- **可重复性与可追溯**：算法能让解释过程更标准化。

因此，算法在临床变异解释中不只是"计算工具"，而是**证据整合与决策支持的框架**。
