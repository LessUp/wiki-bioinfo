---
sidebar_position: 7
description: ATAC-seq footprinting 算法的核心统计思想，包括 Tn5 插入偏好校正和足迹检测的教学性解释。
pagination_label: Footprinting 算法
---
...
<SummaryBox
  summary="Footprinting 算法用于从 ATAC-seq 数据中推断转录因子的实际结合位点，通过检测 motif 区域内 Tn5 插入的显著下降来识别蛋白保护区域。"
  bullets={[
    '核心思想是转录因子结合会保护 DNA 免受 Tn5 切割',
    '关键挑战是校正 Tn5 序列偏好和测序噪声',
    '常用方法包括基于插入位点的统计检验和深度学习模型'
  ]}
/>

## 是什么

Footprinting 算法解决以下问题：

> 给定 ATAC-seq 数据和已知的转录因子 motif，识别基因组上哪些 motif 位点实际被转录因子结合，从而形成"足迹"（footprint）。

当转录因子结合到 DNA 上时，它会物理阻碍 Tn5 转座酶的插入，导致 motif 区域的插入信号显著低于两侧。Footprinting 算法通过检测这种模式来推断真实的蛋白结合。

## 要解决什么生物信息学问题

在实际应用中，footprinting 用于：

- 从 ATAC-seq 数据中推断活跃的转录因子
- 识别特定细胞类型的调控程序
- 区分开放染色质区域中真正被占用的位点
- 补充 motif 富集分析，提供结合证据
