...
**核心思想**：在每一步选择当前最优的局部选择，希望这能导致全局最优解。
...
**问题**：通过反转操作将一个基因组排列转换为目标排列，使反转次数最少。

**简单贪心策略**：每一步增加已排序前缀的长度。

**问题**：这个策略并不总是最优。例如，排列 `6 1 2 3 4 5`：
- 贪心算法需要 5 步
- 但实际上只需要 2 步：`6 1 2 3 4 5 → 5 4 3 2 1 6 → 1 2 3 4 5 6`

**改进策略**：使用断点（breakpoints）作为进度度量，而不是简单的前缀长度。每次反转选择能最大程度减少断点数量的操作。

详见[基因组重排](./genome-rearrangements.md)。

#### 2. 找零问题（Change Problem）

**问题**：给定面额和金额，用最少数量的硬币找零。

**贪心策略**：每次选择不大于剩余金额的最大面额。

**局限性**：对于某些面额系统，贪心算法会失败。例如，面额 {25, 20, 10, 5, 1}，金额 40：
- 贪心算法：25 + 10 + 5 = 3 枚硬币
- 最优解：20 + 20 = 2 枚硬币

#### 3. Motif Finding 中的贪心方法

**策略**：从随机初始解开始，每次改进一个序列的 motif 位置，使用 profile 概率指导选择。
...
当问题很难精确求解时，近似算法提供性能保证。

**近似比（Approximation Ratio）**：
```
近似比 = A(π) / OPT(π)
```
其中 A(π) 是算法返回的解，OPT(π) 是最优解。

**示例**：反转排序问题的改进贪心算法有近似比 4，即返回的解最多是最优解的 4 倍。目前最好的算法近似比为 1.375。

详见 [近似算法](./approximation-algorithms.md)。
