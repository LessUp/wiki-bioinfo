...
<SummaryBox
  summary="限制性图谱（restriction mapping）是通过限制性酶切实验确定DNA分子上酶切位点位置的技术。核心计算问题是根据片段长度数据推断酶切位点的顺序和位置。"
  bullets={[
...
给定一组切割位点 X = {x₁, x₂, ..., xₙ}（假设在一条直线上，x₁ = 0），定义所有点对之间的距离集合：
...
最直观的方法是枚举所有可能的位点组合，计算它们的距离集合，与输入比较。

```
BRUTEFORCEPDP(L)
1 for each possible set X
2   if ΔX = L
3     return X
```

**问题**：搜索空间太大，对于 n 个位点，有指数级可能。
...
- [穷举搜索](/docs/foundations/algorithms-and-complexity)
- [分支定界](/docs/foundations/algorithms-and-complexity)
- [动态规划](/docs/applications/dynamic-programming)
- [基因组组装](/docs/assembly/de-bruijn)
