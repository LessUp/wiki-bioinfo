...
多组学整合是指同时分析来自不同生物学层次的数据（如基因组、转录组、表观组、蛋白组、代谢组等），以获得比单一组学更全面的生物学理解。

常见的多组学场景包括：

- **同一样本的多组学**：同一个病人的 WGS + RNA-seq + 蛋白组；
- **同一细胞的多组学**：单细胞 multiome（如 scRNA-seq + scATAC-seq）；
- **同一队列的不同组学**：不同样本分别测不同组学，但属于同一研究；
- **公共数据整合**：整合不同研究或数据库的组学数据。

整合的目标通常包括：

- **预测**：用多组学特征预测表型、生存或治疗反应；
- **分群**：基于多组学数据识别疾病亚型或细胞状态；
- **机制解释**：理解不同组学层次之间的因果关系或调控关系。
...
假设研究某种癌症的生存期预测，有同一批 100 个病人的 WGS、RNA-seq 和蛋白组数据。
...
不一定。如果整合方法选择不当，或者数据质量不匹配，整合可能引入更多噪声。需要用独立数据验证整合是否真的带来提升。

### 忽略批次效应

不对。不同组学、不同平台、不同批次之间的批次效应可能在整合时被放大，导致虚假关联。整合前必须做好批次校正。
