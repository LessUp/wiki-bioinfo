...
MOFA+（Multi-Omics Factor Analysis Plus）是用于多组学数据整合的贝叶斯因子分析方法。

其核心假设是：观测到的多组学数据由一组共享的潜在因子（latent factors）驱动，每组学对这些因子有不同的敏感度（权重）。
...
其中：
- $X^{(k)} \in \mathbb{R}^{n \times p_k}$ 是组学 $k$ 的观测数据
- $Z \in \mathbb{R}^{r \times n}$ 是共享潜在因子矩阵（$r$ 个因子，$n$ 个样本）
- $W^{(k)} \in \mathbb{R}^{p_k \times r}$ 是组学 $k$ 的因子载荷矩阵
- $\tau^{(k)}$ 是组学 $k$ 的噪声参数
- $\epsilon^{(k)}$ 是观测噪声
