---
sidebar_position: 8
description: Neighbor-Joining算法详解：不依赖分子钟假设的距离矩阵建树方法，理解Q矩阵和最小化树长原理。
pagination_label: Neighbor-Joining算法
---
...
Neighbor-Joining（NJ）是一种自底向上的距离矩阵建树算法，由Saitou和Nei在1987年提出。

与UPGMA不同，NJ的关键特点是：
- **不依赖分子钟假设**：允许不同分支以不同速率演化
- **最小化总树长**：通过选择使总树长最小的邻居对来构建树
- **产生无根树**：结果是一棵无根的系统发育树
