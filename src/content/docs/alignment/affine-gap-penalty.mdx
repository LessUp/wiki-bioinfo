...
这一模型由 Gotoh（1982）首次在 $O(nm)$ 时间内高效实现，因此也被称为 **Gotoh 算法**（详见 [Gotoh 算法](./gotoh/)）。
...
Gotoh（1982）证明了，在仿射罚分下，可以将空间复杂度从 $O(mn)$ 优化到 $O(\min(m,n))$，同时保持时间复杂度为 $O(mn)$。关键观察是：$I_{i,j}$ 只依赖于 $I_{i,j-1}$ 和 $M_{i,j-1}$，$D_{i,j}$ 只依赖于 $D_{i-1,j}$ 和 $M_{i-1,j}$。因此，只需要保留当前列和前一列的值即可。但空间优化的代价是无法直接回溯——需要使用 Hirschberg 算法的分治策略来恢复比对。详见 [Gotoh 算法](./gotoh/)。
