...
- **挑战**：当你在搜索拥有数千亿碱基的 GenBank 数据库时，对每一条序列运行动态规划在计算上是不可接受的。假设查询序列长度为 300bp，数据库总长为 $10^{11}$ bp，使用 Smith-Waterman 需要 $3 \times 10^{13}$ 次基本操作，在单机上可能需要数天。
...
- 查询序列不能太短。对于 blastn，查询序列通常应至少 50-100bp；对于 blastp，至少 15-20 个氨基酸。
...
E-value 衡量的是**统计显著性**，而非同源性的强度。一个短的完美匹配可能有很低的 E-value，但不一定代表生物学上有意义的同源关系。E-value 受查询序列长度、数据库大小和打分体系的影响，在不同搜索之间需要谨慎比较。
...
- 使用 PSI-BLAST 进行迭代搜索。
- 使用 Smith-Waterman 精确搜索（如 SSEARCH）。
- 换用更宽松的替换矩阵（如 BLOSUM45）。

### BLAST 结果中的最高分就是最相关的同源

不一定。最高分可能来自一个包含低复杂度区域的假阳性匹配。需要结合 E-value、覆盖度（Query Coverage）、一致性百分比和功能注释等综合判断。

### blastn 和 blastp 可以互换

核酸和蛋白质水平的比对反映了不同的进化信号。蛋白质比对通常比核酸比对更敏感，因为蛋白质有 20 种氨基酸（信息量更大），且许多同义突变在核酸水平会降低信号。在寻找远缘同源时，blastx 或 blastp 通常比 blastn 更有效。
