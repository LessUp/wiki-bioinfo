...
### 时间复杂度 $O(n^2 / \log n)$ 意味着对所有 $n$ 都更快

Four Russians 技术只在序列足够长时才有加速效果。当 $n$ 较小时（如 $n < 1000$），预计算查找表的开销可能超过主算法的收益。理论分析中 $t = \Theta(\log n)$ 的最优块大小选择，在实际中往往不适用——真实的最佳 $t$ 通常在 4-8 之间，远小于理论值。

### Four Russians 技术与 BWT 索引或 seed-and-extend 是同一种思路

不是。Four Russians 是对动态规划本身的加速，通过预计算-查表减少单元格计算量；而 BWT 索引和 seed-and-extend 是启发式方法，通过跳过不可能的区域来减少搜索空间。两者解决的是不同层面的问题：前者优化"已知需要比对的区域"的计算效率，后者优化"哪些区域值得比对"。
