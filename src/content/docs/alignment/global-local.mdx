...
<figure>
  <img src="/img/illustrations/alignment-grid.svg" alt="全局比对 DP 矩阵的网格示意图" />
  <figcaption>全局比对与局部比对的 DP 矩阵填充过程对比</figcaption>
</figure>
...
<ComparisonTable
  ...
  rightTitle="局部比对（SW）"
  rows={[
    { aspect: '**应用场景**', left: '比较近缘物种的同源基因。', right: '寻找结构域、Motif、Read 定位。' },
    { aspect: '**矩阵初始化**', left: '惩罚首行首列（$i \cdot g$）。', right: '首行首列全为 0。' },
    { aspect: '**递推公式**', left: '取三个方向的最大值。', right: '取四个方向（含 0）的最大值。' },
    { aspect: '**回溯位置**', left: '必须从 $(n, m)$ 开始。', right: '从矩阵内任意最大值处开始。' },
    { aspect: '**本例最优得分**', left: '2', right: '8' }
  ]}
/>
