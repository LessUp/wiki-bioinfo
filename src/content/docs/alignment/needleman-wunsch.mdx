...
```mermaid
graph TD
    subgraph DP_Step[当前单元格 F[i][j] 的来源]
    PrevDiag(F[i-1][j-1]) -->|"+s(xi, yj)"| Current(F[i][j])
    PrevUp(F[i-1][j]) -->|"+g"| Current
    PrevLeft(F[i][j-1]) -->|"+g"| Current
    end
    
    PrevDiag -.->|对角线| Current
    PrevUp -.->|垂直| Current
    PrevLeft -.->|水平| Current

    style Current fill:#f96,stroke:#333,stroke-width:4px
```
...
```mermaid
graph BT
    End((F[m][n])) -->|寻找来源| Step1[...]
    Step1 -->|...| Start((F[0][0]))
    
    subgraph Backtrack_Logic[回溯逻辑]
    Direction{哪个方向?}
    Direction -->|对角线| Match[匹配/替换]
    Direction -->|上方| GapX[X序列Gap]
    Direction -->|左方| GapY[Y序列Gap]
    end
```
