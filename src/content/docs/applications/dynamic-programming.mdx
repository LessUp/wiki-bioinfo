...
<SummaryBox
  summary="动态规划是生物信息学最核心的算法范式之一，通过将复杂问题分解为重叠子问题，系统地寻找全局最优解。"
  bullets={[
    '理解动态规划的关键在于设计递推公式',
    '状态定义和边界条件比具体算法更重要',
  ]}
...
<PrerequisitesBox
  items={[
    ...
    { name: '序列比对概述', href: '/wiki-bioinfo/alignment/index' },
...
详见：[Needleman-Wunsch 算法详解](../alignment/needleman-wunsch.mdx)
...
\max_{i<k<j} \{N[i][k] + N[k+1][j]\} & \text{分叉}
\end{cases}
$$
...
不是。大规模序列比对通常使用启发式方法（seed-and-extend），DP 仅用于局部精化。
...
详见：[Smith-Waterman 算法详解](../alignment/smith-waterman.mdx)
- [编辑距离](../alignment/edit-distance.mdx)
- [HMM 与 Viterbi 算法](../models/viterbi-forward-backward.mdx)
- [算法总览](./algorithms-overview.mdx)

<RelatedLinks
  links={[
    { title: 'Needleman-Wunsch 算法', to: '/wiki-bioinfo/alignment/needleman-wunsch', description: '全局序列比对的经典 DP 算法' },
    { title: 'Smith-Waterman 算法', to: '/wiki-bioinfo/alignment/smith-waterman', description: '局部序列比对的 DP 算法' },
    { title: '编辑距离', to: '/wiki-bioinfo/alignment/edit-distance', description: '基于 DP 的字符串编辑距离计算' },
    { title: 'HMM 与 Viterbi 算法', to: '/wiki-bioinfo/models/viterbi-forward-backward', description: 'HMM 中的 DP 推断算法' },
