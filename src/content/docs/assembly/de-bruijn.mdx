...
description: 基于 k-mer 与 de Bruijn 图的基因组组装方法：图构建、遍历算法及与 OLC 方法的比较。
---
...
<figure>
  <img src="/wiki-bioinfo/img/illustrations/de-bruijn-details.svg" alt="de Bruijn graph 构建与干扰结构示意图" />
  <figcaption>左：从序列到 k-mer 的滑动窗口分解；右：顶点（（k-1）-mer）与边（k-mer）的映射关系；下：图中常见的错误结构（Tip 和 Bubble）。</figcaption>
</figure>
...
<RelatedLinks
  links={[
    { title: 'OLC：Overlap-Layout-Consensus', to: '/wiki-bioinfo/assembly/olc', description: '基于 read-read 重叠的经典组装范式' },
    { title: '重复序列、分叉与图清理', to: '/wiki-bioinfo/assembly/repeats-and-graph-cleaning', description: '组装图中的 tips、bubbles 与重复解析' },
    { title: 'k-mer 与序列表示', to: '/wiki-bioinfo/sequence/kmers', description: 'k-mer 的概念、计数与在组装中的应用' },
    { title: '图遍历算法', to: '/wiki-bioinfo/assembly/graph-traversal-algorithms', description: 'DFS、BFS 及其在组装路径恢复中的角色' },
    { title: '组装评估', to: '/wiki-bioinfo/assembly/assembly-evaluation', description: '连续性、完整性与正确性的多维度评估' },
  ]}
/>
