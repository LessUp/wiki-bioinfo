...
<PrerequisitesBox
  links={[
    ...
    { title: '最短超串问题', href: '/wiki-bioinfo/assembly/shortest-superstring' },
  ]}
/>
...
探针长度 l 增大确实提高了杂交特异性，但代价是探针总数呈指数增长（$4^l$），导致芯片成本和复杂度急剧上升。同时，较长的探针意味着更少的 k-mer 种类被检测到，降低了覆盖度。l 的选择是一个特异性与覆盖度之间的权衡，并非越大越好。

### SBH 已经被完全淘汰

虽然 SBH 作为从头测序方法确实被淘汰，但其核心思想——通过 k-mer 谱重构序列——在现代方法中以不同形式延续。k-mer 频谱分析用于基因组特征估计，de Bruijn 图组装继承了欧拉路径的建模思想，而 DNA 微阵列（基因芯片）仍在基因分型和基因表达分析中广泛应用。
...
- [k-mer](../sequence/kmers.mdx)

<RelatedLinks
  links={[
    { title: 'de Bruijn graph 组装', to: '/wiki-bioinfo/assembly/de-bruijn', description: 'SBH 欧拉路径思想在现代 de Bruijn 图组装中的延续' },
    { title: '最短超串问题', to: '/wiki-bioinfo/assembly/shortest-superstring', description: '与 SBH 密切相关的理论组装模型' },
    { title: '图遍历算法', to: '/wiki-bioinfo/assembly/graph-traversal-algorithms', description: '欧拉路径与哈密顿路径的遍历算法' },
    { title: 'k-mer 与序列表示', to: '/wiki-bioinfo/sequence/kmers', description: 'k-mer 分析在 SBH 和现代组装中的应用' },
    { title: '概率、图与动态规划预备', to: '/wiki-bioinfo/foundations/probability-and-graphs', description: '图论基础与欧拉路径的数学预备' },
  ]}
/>
