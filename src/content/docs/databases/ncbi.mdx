...
<PrerequisitesBox
  items={[
    ...
    {name: '常用数据库与资源总览', href: '/wiki-bioinfo/databases/common-resources'},
  ]}
/>
...
<ComparisonTable
  ...
  rightTitle="GenBank"
  rows={[
    { aspect: '**冗余度**', left: '非冗余，每个分子一个代表序列', right: '冗余，同一序列可能多次提交' },
    { aspect: '**质量**', left: '人工审编 + 自动预测', right: '原始提交，未经统一审核' },
    { aspect: '**用途**', left: '参考序列、标准比对模板', right: '归档所有公开序列' },
    { aspect: '**更新**', left: '定期审编更新', right: '实时接收新提交' }
  ]}
/>
...
<RelatedLinks
  links={[
    ...
    {
      title: 'FASTQ 格式',
      to: '/wiki-bioinfo/formats/fastq-format',
      label: '数据格式',
    ...
    },
    {
      ...
      label: '数据格式',
      description: 'dbSNP rsID 和 ClinVar 注释在 VCF 文件中的表示。',
    },
    {
      title: 'DNA-seq 变异检测总览',
      to: '/wiki-bioinfo/variants/variant-calling-overview',
      label: '应用层',
      description: 'SRA 数据下载后进入 FASTQ → BAM → VCF 变异检测流程。',
    },
  ]}
/>
