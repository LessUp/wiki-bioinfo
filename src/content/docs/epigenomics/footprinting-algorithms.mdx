...
```mermaid
graph TD
    subgraph "信号模式 (Footprint Profile)"
    Signal["Tn5 插入信号"] --- Dip["'凹陷' (Footprint)"]
    Signal --- Flank["'侧翼' (High Signal)"]
    end

    subgraph "分子状态"
    TF["转录因子 (TF)"] -- "结合" --> DNA["DNA 序列 (Motif)"]
    DNA -- "物理阻碍" --> Blocked["Tn5 无法切入"]
    Open["侧翼开放染色质"] -- "允许" --> Access["Tn5 正常切入"]
    end

    Dip <==> Blocked
    Flank <==> Access

    style Dip fill:#ffebee,stroke:#b71c1c,stroke-width:2px
    style Flank fill:#e1f5fe,stroke:#01579b
```
