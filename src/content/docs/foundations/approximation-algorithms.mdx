...
<PrerequisitesBox
  items={[
    ...
    { name: '穷举搜索与分支定界', href: '/wiki-bioinfo/foundations/exhaustive-search' },
...
<RelatedLinks
  links={[
    { title: '穷举搜索与分支定界', to: '/wiki-bioinfo/foundations/exhaustive-search', description: '在精确求解不可行时，分支定界和穷举是理解问题难度的基线方法。' },
    { title: '随机化算法', to: '/wiki-bioinfo/foundations/randomized-algorithms', description: '通过引入随机性跳出局部最优，与近似算法互为补充。' },
    { title: 'Motif discovery 的算法路线', to: '/wiki-bioinfo/models/motif-discovery-algorithms', description: 'Motif 发现中的 Gibbs Sampling 等随机化近似方法。' },
    { title: '系统发育树概览', to: '/wiki-bioinfo/phylogeny/index', description: '系统发育树构建是近似算法的重要应用场景。' },
    { title: '多重序列比对', to: '/wiki-bioinfo/alignment/multiple-sequence-alignment', description: '寻找最优多序列比对是经典的 NP-hard 近似问题。' },
  ]}
/>
