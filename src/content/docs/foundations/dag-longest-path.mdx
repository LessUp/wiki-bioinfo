...
<ComparisonTable
  ...
  rightTitle="DAG 最长路径"
  rows={[
    { aspect: '图结构', left: '规则的 $n \times m$ 网格', right: '任意的有向无环图' },
    { aspect: '边方向', left: '只能向南或向东', right: '任意（只要不形成环）' },
    { aspect: '拓扑排序', left: '隐含的（行列顺序）', right: '显式计算或利用 DAG 特性' },
    { aspect: '应用', left: '教学示例、基础 DP', right: '复杂的生物信息学问题' }
  ]}
/>
...
<RelatedLinks
  links={[
    { title: '曼哈顿游客问题', to: '/wiki-bioinfo/foundations/manhattan-tourist', description: 'DAG 最长路径的特例' },
    { title: '动态规划基础', to: '/wiki-bioinfo/foundations/dynamic-programming-basics', description: '算法基础' },
    { title: 'Exon Chaining', to: '/wiki-bioinfo/models/exon-chaining', description: '生物信息学应用' },
    { title: '拓扑排序', to: '/wiki-bioinfo/foundations/graph-algorithms', description: '图算法基础' }
  ]}
/>
