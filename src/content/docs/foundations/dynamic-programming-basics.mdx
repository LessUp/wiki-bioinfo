...
考虑以下 4×4 的网格（简化自 4×5），其中数字表示边的权重。从 (0,0) 出发，垂直边权重依次为 1→6→5→5（向南），水平边权重依次为 3→2→4（向东），各节点处还有交叉边权重。
...
填表只是实现手段。动态规划的核心是三个要素：最优子结构、重叠子问题、无后效性。很多问题可以设计出 DP 方案但不一定需要显式填表（如带记忆化的递归）。反之，盲目填表而不验证这三个前提条件，可能导致错误结果。

### 所有递归问题都能用动态规划优化

只有存在"重叠子问题"时，动态规划才有优势。例如，归并排序的递归子问题是相互独立的（不重叠），此时分治比 DP 更自然。判断一个递归问题是否适合 DP，关键是看子问题的解是否被多次重复计算。

### DP 表的值就是最优解

DP 表存储的通常只是最优值（如最长路径的权重），而非最优解本身。要恢复具体的最优路径或方案，需要在填表时记录决策方向（回溯指针），然后通过回溯重建。在空间受限时，回溯需要特殊的技巧（如 Hirschberg 算法）。

### 动态规划的时间复杂度总是 O(n²)

DP 的时间复杂度取决于状态空间大小和每个状态的转移代价。一维 DP 可以是 O(n)（如换钱问题），二维 DP 是 O(nm)（如序列比对），三维 DP 可以是 O(nmp)（如剪接比对）。不能将 DP 与 O(n²) 简单等价。

<RelatedLinks
  links={[
    { title: 'Needleman-Wunsch 全局比对', to: '/wiki-bioinfo/alignment/needleman-wunsch', description: '全局比对的经典 DP 算法，直接源自曼哈顿游客问题的思路。' },
    { title: 'Smith-Waterman 局部比对', to: '/wiki-bioinfo/alignment/smith-waterman', description: '在 DP 框架下寻找最优局部相似区域。' },
    { title: '编辑距离', to: '/wiki-bioinfo/alignment/edit-distance', description: '编辑距离是 DP 的另一个经典应用，与序列比对紧密相关。' },
    { title: '仿射空隙罚分', to: '/wiki-bioinfo/alignment/gotoh', description: '对基础 DP 比对的扩展，引入更合理的空隙罚分模型。' },
    { title: '分治算法基础', to: '/wiki-bioinfo/foundations/divide-and-conquer', description: 'Hirschberg 算法展示了分治如何优化 DP 的空间复杂度。' },
  ]}
/>
