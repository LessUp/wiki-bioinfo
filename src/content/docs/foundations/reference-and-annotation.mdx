...
```mermaid
graph TD
    subgraph "Reference Framework"
    Ref["Reference Sequence (FASTA)"]
    end
    
    subgraph "Observations & Annotations"
    Reads["Sample Reads (BAM)"]
    Vars["Variants (VCF)"]
    Anno["Features (GTF)"]
    end
...
```mermaid
graph LR
    subgraph "0-based (BED/BAM)"
    B_Start[0] --- B_End[100]
    B_Note["Half-open [0, 100)"]
    end
    
    subgraph "1-based (VCF/GFF)"
    G_Start[1] --- G_End[100]
    G_Note["Closed [1, 100]"]
    end
    
    B_Note ---|"Length = 100"| G_Note
```
