...
<PrerequisitesBox
  items={[
    ...
    { name: '算法复杂度', href: '/wiki/foundations/algorithms-and-complexity' },
  ]}
/>
...
<RelatedLinks
  links={[
    { title: '穷举搜索', to: '/wiki/foundations/exhaustive-search', description: '穷举搜索的基本原理和复杂度分析' },
    { title: '算法复杂度', to: '/wiki/foundations/algorithms-and-complexity', description: '理解算法效率的理论框架' },
    { title: '分支定界', to: '/wiki/foundations/branch-and-bound', description: '组合优化中的剪枝策略' },
  ]}
/>
...
虽然高通量测序在很大程度上取代了限制性图谱作为基因组作图的主要手段，但限制性图谱仍用于验证组装结果（特别是结构变异）、研究表观遗传学（如甲基化敏感的限制性酶）以及在某些快速诊断场景中。它是一种互补而非被淘汰的技术。

<RelatedLinks
  links={[
    { title: '穷举搜索', to: '/wiki/foundations/exhaustive-search' },
    { title: '换钱问题', to: '/wiki/foundations/change-problem', description: '另一个动态规划入门案例' },
    { title: '基因组组装', to: '/wiki/assembly/olc', description: '大规模图谱构建的现代方法' }
  ]}
/>
