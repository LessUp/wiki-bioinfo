...
<RelatedLinks
  links={[
    { title: '贝叶斯推断基础', to: '/wiki-bioinfo/models/bayesian-inference', description: '贝叶斯推断提供处理隐变量的另一种视角，与 EM 形成互补。' },
    { title: 'Motif discovery 的算法路线', to: '/wiki-bioinfo/models/motif-discovery-algorithms', description: 'MEME 等工具使用 EM 算法寻找转录因子结合位点。' },
    { title: '隐马尔可夫模型', to: '/wiki-bioinfo/models/hmm', description: 'Baum-Welch 算法是 EM 在 HMM 参数估计上的特例。' },
    { title: 'PWM 与 PSSM', to: '/wiki-bioinfo/models/pwm-pssm', description: 'EM 算法的输出常为 PWM/PSSM 形式的 motif 模型。' },
    { title: 'Viterbi、Forward 与 Backward', to: '/wiki-bioinfo/models/viterbi-forward-backward', description: 'EM 的 M 步依赖 Forward-Backward 计算的后验概率。' },
  ]}
/>
