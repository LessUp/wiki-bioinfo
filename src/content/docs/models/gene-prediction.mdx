...
<PrerequisitesBox
  links={[
    ...
    { title: '参考基因组、坐标系统与注释', href: '/wiki-bioinfo/foundations/reference-and-annotation' },
  ]}
/>
...
<RelatedLinks
  links={[
    { title: '隐马尔可夫模型', to: '/wiki-bioinfo/models/hmm', description: 'HMM 是基因预测的核心概率模型，用于序列状态分段。' },
    { title: 'Viterbi、Forward 与 Backward', to: '/wiki-bioinfo/models/viterbi-forward-backward', description: 'Viterbi 算法用于寻找最优的 exon/intron 状态路径。' },
    { title: 'Profile HMM', to: '/wiki-bioinfo/models/profile-hmm', description: 'Profile HMM 与基因预测 HMM 共享状态建模思想，但应用目标不同。' },
    { title: '参考基因组、坐标系统与注释', to: '/wiki-bioinfo/foundations/reference-and-annotation', description: '基因预测的输出是基因组注释的核心组成部分。' },
    { title: '转录组概览', to: '/wiki-bioinfo/transcriptomics/index', description: 'RNA-seq 等转录组数据是基因预测的重要证据来源。' },
  ]}
/>
