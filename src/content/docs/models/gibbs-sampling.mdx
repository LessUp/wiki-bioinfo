...
<PrerequisitesBox
  items={[
    { name: 'Motif Finding', href: '/wiki/models/motif-finding' },
    { name: '随机算法', href: '/wiki/foundations/randomized-algorithms' },
    { name: '位置权重矩阵', href: '/wiki/models/pwm-pssm' }
  ]}
/>
...
### Profile 矩阵的得分越高，找到的 motif 越有生物学意义

Profile 得分（如信息含量）只衡量序列的统计保守性，不直接反映生物学功能。一组高度保守但缺乏功能关联的序列（如来自同一个基因家族的启动子区域中的某个非功能性保守元件）可能产生高得分。反过来，某些功能重要的 motif（如仅在某几个关键基因的启动子中出现）可能因为样本量不足而得分较低。生物学意义的判断需要结合实验验证和功能注释，而非仅看统计得分。

### 迭代次数越多，burn-in 期可以忽略

Burn-in 期的作用是让马尔可夫链"忘记"初始化的影响，进入平稳分布。如果 burn-in 设置得太短，后续采样中会混入初始化偏差。关键在于 burn-in 应该足够长使得链"混合良好"，而非简单地占总迭代的一定比例。在实践中，可以通过监控目标函数的收敛轨迹来判断 burn-in 是否充足。如果不同随机重启的最终结果差异很大，通常说明 burn-in 不足或问题本身存在多个局部最优。

<RelatedLinks
  links={[
    { title: 'Motif Finding', to: '/wiki/models/motif-finding', description: 'Motif 寻找问题' },
    { title: 'PWM/PSSM', to: '/wiki/models/pwm-pssm', description: '位置权重矩阵' },
    { title: '随机算法', to: '/wiki/foundations/randomized-algorithms', description: '随机算法基础' },
    { title: '随机投影', to: '/wiki/foundations/randomized-algorithms', description: '另一种随机 motif 寻找方法' }
  ]}
/>
