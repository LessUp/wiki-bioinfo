...
### 中间字符串问题 (Median String Problem)

Motif 寻找也可以转化为寻找一个**中间字符串 (Median String)** $v$，使得所有输入序列中与 $v$ 距离最近的 $l$-mer 的距离之和最小。

$$d(v, \text{DNA}) = \sum_{i=1}^t \min_{s_i} d(v, s_i)$$

这是一个穷举搜索问题，复杂度为 $O(4^l \cdot n \cdot t)$。

### 贪心 Motif 搜索 (Greedy Motif Search)
