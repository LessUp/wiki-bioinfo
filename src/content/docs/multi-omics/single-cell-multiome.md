...
**事实**：Multiome 数据仍然受技术因素影响：
- 转座酶批次差异
- 文库制备时间差异
- 测序平台差异

**建议**：仍需要进行批次校正（如 Harmony、scVI）。

### 误区 2：RNA 和 ATAC 一定一一对应

**事实**：调控变化与表达变化可能存在时间滞后：
- 染色质先开放，转录后激活
- 转录已停止，但染色质仍开放
- 转录后调控的影响

**建议**：分析时考虑 lag time，不要期待完全同步。

### 误区 3：多模态一定优于单模态

**事实**：优势取决于研究问题：
- 如果关注转录调控，Multiome 有价值
- 如果仅关注细胞分型，单 RNA 可能足够
- 数据质量差时，多模态可能引入更多噪声

**建议**：根据科学问题决定是否值得投入 Multiome。
