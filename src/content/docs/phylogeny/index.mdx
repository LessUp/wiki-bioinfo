...
1. **[层次聚类](./hierarchical-clustering/)** — 理解树状结构的基本构建方法
2. **[k-means 在生物信息学中的应用](./k-means-bioinformatics/)** — 理解聚类与分区概念
3. **[多序列比对（MSA）](../alignment/multiple-sequence-alignment/)** — 获得系统发育分析的输入数据
4. **[距离方法概览](./distance-methods/)** — 理解距离矩阵方法的一般框架
5. **[UPGMA算法](./upgma/)** — 学习基于分子钟假设的层次聚类
6. **[Neighbor-Joining算法](./neighbor-joining/)** — 学习不依赖分子钟假设的距离法
7. **[加法系统发育（Additive Phylogeny）](./additive-phylogeny/)** — 理解距离矩阵与树的理论关系
8. **[Parsimony](./parsimony/)** — 学习基于最小进化的位点方法
9. **[Maximum Likelihood](./maximum-likelihood/)** — 学习基于统计模型的现代方法
...
<SectionNavigator
  items={[
    {
      title: '层次聚类',
      to: '/wiki-bioinfo/phylogeny/hierarchical-clustering',
      badge: '表达数据',
      meta: 'dendrogram',
    ...
    },
    {
      ...
      to: '/wiki-bioinfo/phylogeny/k-means-bioinformatics',
      badge: '表达数据',
    ...
    {
      title: '距离方法概览',
      to: '/wiki-bioinfo/phylogeny/distance-methods',
      badge: '起点',
      meta: 'distance-based',
    ...
    },
    {
      ...
      to: '/wiki-bioinfo/phylogeny/additive-phylogeny',
      badge: '理论骨架',
    ...
    {
      title: 'Parsimony',
      to: '/wiki-bioinfo/phylogeny/parsimony',
      badge: '位点方法',
    ...
    },
    {
      ...
      to: '/wiki-bioinfo/phylogeny/maximum-likelihood',
      badge: '模型方法',
      meta: 'likelihood',
...
- [RNA-seq 工作流概览](../workflows/rna-seq/) 中的表达矩阵常用层次聚类或 k-means 做初步模式探索；
- [TPM、FPKM、CPM 与有效长度](../transcriptomics/tpm-fpkm-cpm/) 提供 expression matrix 的输入背景；
- 树状图在 expression analysis 中更偏相似性可视化，不应直接等同于真实进化树。

## 与其他板块的连接

- 上游依赖 [多序列比对（MSA）](../alignment/multiple-sequence-alignment/) 和 [序列比对](../alignment/index/) 提供同源位点基础；
- 作为 [分析方向与案例](../applications/index/) 中的一个专题入口；
