...
| 前体质量过滤 | $O(|\mathcal{P}|)$ | 按母离子质量筛选候选肽段 |
| 评分计算 | $O(n \cdot k \cdot |\mathcal{P}_s|)$ | $n$ 张谱图，每张与 $|\mathcal{P}_s|$ 个候选比较，$k$ 为碎片数量 |
| FDR 计算 | $O(n \cdot \log n)$ | 排序 + 累积计算 |

其中 $|\mathcal{P}_s|$ 是经过质量过滤后的候选数量。实践中，使用前体索引（Precursor Index）可以将搜索速度提高数个数量级。
...
数据库搜索鉴定的是**肽段**，而用户通常关心的是**蛋白质**。从肽段到蛋白质的推断存在歧义性：一组肽段可能对应多个蛋白质（如旁系同源基因的产物共享部分肽段）。简单地将所有包含至少 2 个唯一肽段的蛋白报告为"已鉴定"会导致：
- 假阳性蛋白被过度报告。
- 蛋白质之间的从属关系（如某个蛋白的所有肽段都是另一个蛋白的子集）被忽略。

工具如 ProteinProphet 和 Fido 使用概率模型来量化蛋白推断的不确定性。

### 9.3 Decoy 数据库构建不当

Decoy 数据库的质量直接影响 FDR 估计的准确性。常见问题包括：
- **使用 Shuffled 序列**：随机打乱可能产生不符合真实蛋白质特征的序列（如不自然的氨基酸组成），导致 Decoy 的匹配难度偏低，FDR 被低估。
- **Target-Decoy 混合不充分**：如果 Target 和 Decoy 数据库在序列空间上存在系统差异，搜索算法可能利用这种差异进行"作弊"。
- **推荐做法**：使用 Reversed 序列作为 Decoy，并确保在搜索时 Target 和 Decoy 是混合在一起的。

### 9.4 搜索参数过宽或过窄

- **过窄**：如前体质量容忍度设为 1 ppm 而实际仪器的质量精度为 5 ppm，会导致大量真实肽段因质量不匹配而被遗漏。
- **过宽**：如允许 5 种可变修饰且每个肽段最多 3 个修饰位点，搜索空间可能膨胀数百万倍，不仅极大增加计算时间，还会显著降低鉴定的特异性。
- **建议**：根据仪器的实际性能设置参数，并通过目标搜索（如先用窄参数搜索，再用宽参数补搜未匹配的谱图）来平衡灵敏度和特异性。
