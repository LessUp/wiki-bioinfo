...
### 5.2 TMT/iTRAQ 的比例压缩效应

等质量标签的一个固有问题是**比例压缩（Ratio Compression）**。当同一个 MS1 前体离子中混合了来自不同标记通道的肽段时，MS2 碎裂可能同时"看到"多个前体离子，导致报告离子强度被"稀释"，真实的倍数差异被压缩趋向于 1。解决方案包括：

- 使用 MS3 扫描策略（SPS-MS3），在 MS2 选择碎片后，再次碎裂 MS2 碎片以获得更纯净的报告离子信号。
- 减少每个 MS1 窗口中的共洗脱肽段数量（如使用更窄的 MS1 隔离窗口）。

### 5.3 忽视蛋白推断的不确定性

质谱数据鉴定的是肽段，而非蛋白质。从肽段到蛋白质的推断（Protein Inference）存在不确定性：一组肽段可能对应多个蛋白质（蛋白家族）。直接忽略这种不确定性可能导致：

- 假阳性蛋白被错误地报告为"已鉴定"。
- 差异分析中蛋白丰度的估计偏差。

工具如 Fido 和 ProteinProphet 提供了基于概率模型的蛋白推断方法，可以给出每个蛋白质鉴定的置信度。

### 5.4 过度依赖单一定量方法

每种定量策略都有其适用范围和偏差来源。将不同方法的结果进行交叉验证（如 LFQ 结果与 TMT 结果的一致性检验）可以显著提高结论的可靠性。
