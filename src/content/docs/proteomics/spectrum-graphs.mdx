...
| 误区 | 正确理解 |
|------|---------|
| 谱图图总能唯一确定肽段序列 | 实际中可能存在多条得分相近的路径，导致序列歧义 |
| 完整谱图在实际中很常见 | 实验谱图通常不完备，某些离子类型可能完全缺失 |
| 只需考虑 b-ions | 实际应用需要同时考虑多种离子类型（b、y、a 等） |
| 最长路径总是正确答案 | 对于不完备谱图，需结合概率评分；最长边数路径不一定最优 |
| De novo 测序已取代数据库搜索 | 两者各有优势：de novo 发现新序列，数据库搜索更可靠、快速 |
