...
<PrerequisitesBox
  items={[
    { name: '精确模式匹配', href: '/wiki/sequence/exact-string-matching' },
...
**问题**：将 100bp 的 Illumina reads 比对到人类基因组（3Gb），允许最多 2 个错配。
...
### 动态规划是"精确"的，而启发式方法是"近似"的

这种区分容易产生误导。动态规划确实能在给定得分矩阵和罚分方案下找到数学上的最优比对，但得分方案本身就是一个参数化的模型——不同的得分矩阵（如 BLOSUM62 vs PAM250）会产生不同的"最优"比对。启发式方法（如 BLAST）牺牲的是搜索的完备性（可能遗漏极弱的相似性），而非比对本身的精度——在种子扩展阶段仍使用精确的动态规划。

### q-gram 过滤的鸽巢原理保证了不会遗漏任何匹配

鸽巢原理只保证了至少有一个 q-gram 片段精确匹配，前提是模式被均匀分割且差异数为 k。但在实际实现中，如果 q-gram 索引本身存在错误（如参考序列中的变异），或 k 的估计不准确，仍可能遗漏真实匹配。此外，鸽巢原理对编辑距离（含插入/删除）的保证弱于对汉明距离的保证，因为 indel 会改变 q-gram 的边界。

<RelatedLinks
  links={[
    { title: '精确模式匹配', to: '/wiki/sequence/exact-string-matching', description: '基础匹配算法' },
    { title: 'BLAST', to: '/wiki/sequence/blast', description: '实用的近似匹配工具' },
    { title: '编辑距离', to: '/wiki/alignment/global-local', description: '距离度量基础' },
    { title: 'Suffix Tree', to: '/wiki/sequence/suffix-tree', description: '索引结构' }
  ]}
/>
