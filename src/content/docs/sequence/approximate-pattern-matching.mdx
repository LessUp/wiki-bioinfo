---
description: 近似模式匹配（Approximate Pattern Matching）允许在文本中查找与模式串有少量差异的匹配，是生物信息学中处理测序错误和变异的核心算法。
title: "近似模式匹配"
---
...
<SummaryBox
  summary="近似模式匹配（Approximate Pattern Matching）允许在文本中查找与模式串有最多 k 个差异的匹配，是处理测序错误、基因变异和进化分歧的核心算法工具。"
  bullets={[
    '定义：允许最多 k 个错配/插入/删除的匹配',
    '汉明距离：只允许替换操作',
    '编辑距离：允许替换、插入、删除',
    '是 BLAST 等数据库搜索工具的理论基础'
  ]}
/>
...
- **1965**：Levenshtein 提出编辑距离
- **1970s**：动态规划算法成熟
- **1980s**：Myers 和 Miller 开发线性空间算法
- **1990**：BLAST 算法发布，革命性地加速了数据库搜索
- **1990s-2000s**：后缀数组、BWT/FM-index 用于高效近似匹配
- **2009**：BWA 等工具将 BWT 引入 NGS 比对
