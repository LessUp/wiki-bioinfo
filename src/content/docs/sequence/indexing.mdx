...
- 重复序列分析需要最长重复子串查询 → Suffix Tree。

### 构建索引不值得

当查询次数 $q$ 足够多时，$q \cdot O(nm)$（无索引）远大于 $O(n)$（构建）$+ q \cdot O(m)$（查询）。对于典型的 Illumina 测序数据，$q$ 可达 $10^8$ 以上，构建索引的投入是值得的。
