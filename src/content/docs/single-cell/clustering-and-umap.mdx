...
<ComparisonTable
  ...
  rightTitle="Leiden"
  rows={[
    { aspect: '**核心策略**', left: '贪心模块度优化', right: '贪心优化 + 局部连接性检查' },
    { aspect: '**已发现问题**', left: '可能产生断开的社区', right: '保证社区内部连通' },
    { aspect: '**分辨率控制**', left: '通过 resolution 参数', right: '通过 resolution 参数' },
    { aspect: '**计算速度**', left: '略快', right: '略慢但更稳定' },
    { aspect: '**推荐场景**', left: '快速探索', right: '正式分析（推荐）' }
  ]}
/>
...
<ComparisonTable
  ...
  rightTitle="t-SNE"
  rows={[
    { aspect: '**理论基础**', left: '黎曼几何与代数拓扑', right: 'KL 散度最小化' },
    { aspect: '**计算速度**', left: '较快', right: '较慢' },
    { aspect: '**全局结构保持**', left: '较好', right: '较差' },
    { aspect: '**可重复性**', left: '较好（随机种子更稳定）', right: '较差' },
    { aspect: '**新数据投影**', left: '支持 transform', right: '不支持（需重新计算）' },
    { aspect: '**参数敏感性**', left: '较低', right: '较高' }
  ]}
/>
...
<figure>
  <img src="/img/illustrations/scrna-umap.svg" alt="scRNA-seq UMAP 聚类可视化：不同细胞类型的二维分布" />
  <figcaption>UMAP 可视化：不同颜色代表不同细胞类型的聚类结果</figcaption>
</figure>
