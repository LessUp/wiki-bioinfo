...
- 忽略 Doublet（双联体）检测。参见 [Doublet 检测](./doublet-detection/)。
...
**常见错误**：
- PCA 使用了错误的主成分数量。参见 [聚类与 UMAP](./clustering-and-umap/) 中的详细讨论。
...
| 分析步骤 | 核心算法 | 对应页面 |
|---------|---------|---------|
| Barcode 分配 | 序列比对、白名单过滤 | [细胞 Barcode 与 UMI](./cell-barcode-umi/) |
| UMI 去重 | 邻域聚类、方向性合并 | [细胞 Barcode 与 UMI](./cell-barcode-umi/) |
| 降维 | PCA、UMAP | [聚类与 UMAP](./clustering-and-umap/) |
| 聚类 | Leiden/Louvain 图社区检测 | [聚类与 UMAP](./clustering-and-umap/) |
| 轨迹推断 | 伪时间、RNA Velocity | [轨迹推断](./trajectory-analysis/) |
| Doublet 检测 | 人工模拟、k-NN 评分 | [Doublet 检测](./doublet-detection/) |
...
- **轨迹推断**：追踪细胞分化的动态过程。参见 [轨迹推断](./trajectory-analysis/)。
- **细胞通讯分析**：推断细胞间的配体-受体互作（如 CellChat、NicheNet）。
- **空间转录组整合**：将 scRNA-seq 数据映射到空间坐标。参见 [空间去卷积](../spatial/deconvolution-and-mapping/)。
