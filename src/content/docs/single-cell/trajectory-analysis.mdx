...
- **"所有基因都应纳入 velocity 计算"**
  - 只有表达量足够高且具有显著剪接动力学变化的基因才应纳入。低表达基因的 velocity 估计不可靠。

- **"轨迹方法可以处理所有类型的动态过程"**
  - 大多数轨迹方法假设发育是单向的。对于可逆的状态转换（如细胞激活/静息），需要专门的工具（如 Palantir 的扩散伪时间）。

- **"分支点一定是生物学上的命运决定点"**
  - 算法检测到的分支可能是技术假象（如 batch effect）。应通过分支点附近的差异表达基因和已知生物学知识进行验证。
