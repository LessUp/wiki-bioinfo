...
等价类数量少意味着大量 reads 被归入相同的兼容性集合，这虽然提高了计算效率，但也意味着这些 reads 无法被区分。极端情况下，如果所有 reads 都兼容所有转录本（等价类只有一个），定量结果将完全退化。等价类数量是参考注释复杂度和序列相似性的反映，而非定量质量的指标。

### Pseudo-alignment 可以替代所有比对需求

Pseudo-alignment 仅适用于"已知转录本的表达定量"这一特定场景。如果需要发现新转录本、分析剪接位点、检测基因组变异或研究融合基因，仍必须使用传统 spliced alignment（如 STAR、HISAT2）。将 pseudo-alignment 用于其设计范围之外的任务会导致信息丢失和错误结论。
