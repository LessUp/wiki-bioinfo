...
### 8.2 混淆 FPKM 与 TPM 的数值含义

由于 FPKM 和 TPM 的数值范围相似（都在 0 到数十万之间），很容易混淆两者的含义。关键区别在于：

- FPKM 的总量因样本而异。
- TPM 的总量恒为 $10^6$。

如果直接比较两个样本中同一个基因的 FPKM 绝对值来推断倍数变化，结果可能不准确。**跨样本比较时，始终优先使用 TPM。**

### 8.3 忽略 GC 含量和序列偏好

RNA-seq 文库构建和 PCR 扩增过程中，GC 含量和序列特征会影响 Reads 的产生概率。高 GC 含量的转录本往往被系统性低估。Salmon 和 Kallisto 提供了 GC 偏好校正选项（`--gcBias`），建议在正式分析中启用。

### 8.4 对低表达基因的 TPM 过度解读

TPM 的估计不确定性随表达量降低而急剧增大。一个 TPM = 0.1 的基因可能对应 0 个 Reads，也可能对应 1 个 Reads——两者的统计含义截然不同。在解读低表达基因的 TPM 时，应同时关注原始计数和置信区间。

### 8.5 忽视多联体基因 (Multi-mapping Genes)

当 Reads 比对到多个基因位点（如重复序列区域或高度同源的基因家族）时，不同的定量工具处理方式不同：
- **HTSeq-count**：默认丢弃多重比对 Reads。
- **featureCounts**：提供 `-M` 选项分配多重比对 Reads。
- **Salmon/Kallisto**：通过 EM 算法将多重比对 Reads 按比例分配到兼容的转录本。

处理方式的差异可能导致相同数据在不同工具下的定量结果出现系统性差异。
