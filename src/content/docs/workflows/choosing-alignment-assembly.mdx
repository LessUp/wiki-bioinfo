...
<SummaryBox
  summary="根据研究目标（变异检测、定量分析、基因组重建）和数据特征，选择合适的序列分析策略。"
/>
...
- 如果选择 alignment 路径，继续阅读 [DNA-seq 变异检测总览](../variants/variant-calling-overview)；
- 如果选择定量路径，继续阅读 [RNA-seq 工作流概览](./rna-seq)；
- 如果选择 assembly 路径，继续阅读 [de Bruijn graph 组装](../assembly/de-bruijn)。
